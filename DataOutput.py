...
class DataOutput(object):
    '''
        ...
        如：
        >>>DataOutput(dbname='computer', collection='notebook')
    ...
    def save_info(self, datalist):
        if isinstance(datalist, dict):
            self.collection.insert(datalist)
        elif isinstance(datalist, typing.Iterable):
            for one in datalist:
                if isinstance(one, dict):
                    self.collection.insert(one)
                else:
                    raise ValueError('参数必须是字典类型或其可迭代对象!')
        else:
            raise ValueError('参数必须是字典类型或其可迭代对象!')
...
if __name__ == '__main__':
    adata = DataOutput(dbname='test', collection='test')
